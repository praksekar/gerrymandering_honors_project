...
def district_graph_to_mmds(G: nx.Graph, component_sizes: tuple, rand_cut_iterations: int = 100000, node_repeats: int = 5) -> nx.Graph:
    ...
    raise Exception("partitioning failed after %d random cut iterations after %d node repeats" % (rand_cut_iterations, node_repeats))
...
def main() -> None:
    ...
    district_centroids: dict[int, tuple] = get_district_centroids(district_geo_df)
    ...
    for district_config in district_configs:
        ...
        district_partition = district_graph_to_mmds(district_graph, district_config)
        plot_partition(smd_partition, precinct_geo_df, COLORS)
        nx.draw(district_partition, pos=district_centroids, node_size=[10]*len(district_graph.nodes))
        plt.show()
        plt.clf()
